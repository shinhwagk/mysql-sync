...
def generate_creates(num_creates):
    ...
    for i in range(num_creates):
        ...
        creates.append(f"CREATE TABLE {table_name} (id INT PRIMARY KEY, data VARCHAR(100));")
...
def generate_inserts(tables, num_inserts_per_table):
    ...
    for db_name, table in tables:
        ...
        for i in range(1, num_inserts_per_table + 1):
            ...
            inserts.append(f"INSERT INTO {db_name}.{table} (id, data) VALUES ({i}, '{value}');")
    ...
    for db_name, table in tables:
        if table in id_tracker:
            ...
            for i in range(min(num_updates_per_table, len(ids))):
                ...
                updates.append(f"UPDATE {db_name}.{table} SET data = '{new_value}' WHERE id = {ids[i]};")
    return updates
...
def generate_rename_tables(tables):
    ...
    for db_name, table in tables:
        ...
        if random.choice([True, False]):
            renames.append(f"RENAME TABLE {db_name}.{table} TO {db_name}.{new_table_name};")
        else:
            renames.append(f"ALTER TABLE {db_name}.{table} RENAME TO {db_name}.{new_table_name};")
...
def main():
    ...
    statements = creates + inserts + updates + deletes + replaces + alters + truncates + renames + drop_tables + drop_dbs
