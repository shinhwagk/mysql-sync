...
> 不能使用
```sql
-- root@d08f6caaa6cd mysql-mysqlbinlog-replicaiton]# mysqlbinlog --host=db1 --port=3306 --user=root --password=example --read-from-remote-source=BINLOG-DUMP-GTIDS    --require-row-format --to-last-log --verbose --verbose --idempotent --force-read --print-table-metadata mysql-bin.000001 
...
> 可以使用
```
mysqlbinlog --host=db1 --port=3306 --user=root --password=example --read-from-remote-source=BINLOG-DUMP-GTIDS --compression-algorithms=zstd --to-last-log --verbose --verbose --idempotent --force-read --print-table-metadata mysql-bin.000001 >binlog/from-mysql-bin.000001.BINLOG-DUMP-GTIDS.sql
```

- --connection-server-id
> 避免多个slave有相同的server_id冲突
```
ERROR: Got error reading packet from server: A replica with the same server_uuid/server_id as this replica has connected to the source; the first event 'mysql-bin.000001' at 4, the last event read from './mysql-bin.000003' at 197, the last byte read from './mysql-bin.000003' at 197.
```