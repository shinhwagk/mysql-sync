...
func (bext *BinlogExtract) toMoCh(mo MysqlOperation) {
	...
	bext.metricCh <- MetricUnit{Name: MetricReplExtractorOperationsCache, Value: uint(len(bext.moCh))}
}
...
func (bext *BinlogExtract) handleQueryEvent(e *replication.QueryEvent, eh *replication.EventHeader) error {
	...
	for _, stmt := range stmts {
		...
		case *ast.DropDatabaseStmt:
			...
			bext.metricCh <- MetricUnit{Name: MetricReplExtractorOperationDDLDatabase, Value: 1}
			bext.Logger.Debug("Operation[ddldatabase], Database: %s, Query: %s", database, string(e.Query))
		case *ast.CreateUserStmt, *ast.DropUserStmt, *ast.GrantStmt, *ast.RevokeStmt, *ast.AlterUserStmt, *ast.RenameUserStmt:
			schemaContext := string(e.Schema)
